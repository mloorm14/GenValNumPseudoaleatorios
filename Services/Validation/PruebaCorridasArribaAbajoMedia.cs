...
public sealed class PruebaCorridasArribaAbajoMedia : IPruebaValidacion
{
    ...
    public ResultadoPruebaValidacion Ejecutar(List<double> datos, ParametrosValidacion parametros)
    {
        int n = datos.Count;
        if (n < 2)
            throw new ExcepcionValidacion("Se necesitan al menos 2 datos para la prueba de corridas arriba y abajo de la media.");
        ...
        if (n0 == 0 || n1 == 0)
            throw new ExcepcionValidacion("Todos los valores quedaron del mismo lado de la media (0.5); no se puede aplicar la prueba.");

        int corridas = Estadistica.ContarCorridas(secuencia);

        double mu = (2.0 * n0 * n1) / n + 0.5;
        double varianza = (2.0 * n0 * n1 * (2.0 * n0 * n1 - n)) / (Math.Pow(n, 2) * (n - 1));

        if (varianza <= 0)
            throw new ExcepcionValidacion("La varianza calculada no es válida con estos datos (desbalance extremo entre valores mayores y menores a 0.5).");
